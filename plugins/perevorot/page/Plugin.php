...
class Plugin extends PluginBase
{
    ...
    {
        return [
            'Perevorot\Page\Components\PageSetting' => 'PageSetting',
    ...
    public function boot()
    {
        ...
        {
            \IntegerLog::info('cms.page.display', $url);

            if(env('CACHE_ENABLED', false) && !empty($result) && empty($_POST))
                Cache::put(PageCache::getCacheKey(), $result, PageCache::getCacheMinutes());
        });

        Event::listen('cms.page.beforeDisplay', function($controller, $url, $page)
        {
            $pageObject=PageModel::where('url', str_replace('//', '/', '/'.$url))->first();
            ...
            if($cache && Cache::has(PageCache::getCacheKey()) && empty($_POST))
            ...
            if($user = Auth::getUser()) {
                if($user->lang != $this->locale) {
                    DB::table('users')
                        ->where('id', $user->id)
                        ->update(['lang' => $this->locale]);
                }
            }
