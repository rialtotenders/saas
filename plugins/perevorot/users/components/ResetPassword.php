...
class ResetPassword extends ComponentBase
{
    ...
    {
        \IntegerLog::info('user.restore_password');
        ...
        if (!$user = UserModel::where('username', post('username'))->first()) {
            throw new ApplicationException(trans('rainlab.user::lang.account.invalid_user'));
        }

        $code = implode('!', [$user->id, $user->getResetPasswordCode()]);
        $link = $this->controller->currentPageUrl().'/'.$code;
        ...
        Mail::send('rainlab.user::'.$this->getLocaleForEmail().'mail.restore', $data, function($message) use ($user) {
    ...
    {
        \IntegerLog::info('user.reset_password');
