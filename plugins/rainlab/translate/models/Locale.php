...
class Locale extends Model
{
    ...
    public function makeDefault()
    {
        ...
        $oldDefault = $this->getDefault();
        $newDefault = $this->newQuery()->where('id', $this->id)->first();

        $this->exchangeLocale($oldDefault, $newDefault);
     ...
     * @param $newDefault
     */
    private function exchangeLocale($oldDefault, $newDefault)
    {
        $translates = DB::table('rainlab_translate_attributes')
            ->where('locale', $newDefault->code)
            ->get();

        foreach ($translates as $translate) {
            $model_type = $translate->model_type;

            if (!class_exists($model_type)) {
                continue;
            }

            $model = $model_type::find($translate->model_id);
            ...
            DB::table('rainlab_translate_attributes')
                ...
                ->update([
                    'locale' => $oldDefault->code,
                    'attribute_data' => json_encode($attributeData)
                ]);

            $attributes = json_decode($translate->attribute_data, 1);
            ...
            if ($model->table == 'system_settings') {
                DB::table('system_settings')
                    ->where('id', $model->id)
                    ->where('item', '!=', 'perevorot.rialtotender.setting')
