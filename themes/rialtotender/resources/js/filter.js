;(function ($, LANG, window) {
    ...
    var Search = (function () {
        var search = [];
        var opts = { lines: 17, length: 8, width: 2, radius: 13, scale: 1, corners: 1, color: '#e05a00', opacity: 0.25, rotate: 0, direction: 1, speed: 1, trail: 60, fps: 20, zIndex: 2e9, className: 'spinner', top: '50%', left: '50%', shadow: false, hwaccel: false, position: 'fixed'};
        ...
        var methods = {
            ...
            getContent: function () {
                ...
                $.get(window.location.href, function (content) {
                    ...
                    $('.js-mobile-filter-more-hide').removeClass('show-button');
    ...
    var Filter = function () {
        ...
        var methods = {
            ...
            processFilter: function () {
                ...
                selected = statuses.getSelected();

                if(regions != undefined) {
                    selected = $.extend({}, regions.getSelected(), selected);
                }
                ...
                if (statuses.isInit()) {
                    search.setSearch({
                        'region': regions != undefined ? regions.getSelected() : [],
                        'status': statuses.getSelected(),
                        'edrpou': buyers != undefined ? buyers.getSelected() : [],
                        'query': query.getSelected(),
                        'cpv': categories != undefined ? categories.getSelected() : []
                    });
                }
